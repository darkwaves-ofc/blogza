...
interface AdminHotNewsPreviewProps {
  _pageId?: string;
  page: {
    path: string;
    section: string;
...
export default function AdminHotNewsPreview({
  _pageId,
  page = {
    path: "home",
    section: "hot-news",
  },
  initialTitle = "View the ZZZ Collection!",
  initialContent = JSON.stringify({
    link: "/#",
    title: "Click Here!",
  }),
  initialImages,
}: AdminHotNewsPreviewProps) {
  const {
    loading,
    title,
    setTitle,
  ...
  const handleLinkChange = (newContent: string) => {
    setContent((prevContent) => {
      if (prevContent === undefined) {
        return JSON.stringify({ link: newContent });
      }
      try {
        const parsedContent = JSON.parse(prevContent);
        parsedContent.link = newContent;
  ...
  const handleLinkTitleChange = (newContent: string) => {
    setContent((prevContent) => {
      if (prevContent === undefined) {
        return JSON.stringify({ title: newContent });
      }
      try {
        const parsedContent = JSON.parse(prevContent);
        parsedContent.title = newContent;
        return JSON.stringify(parsedContent);
      } catch (error) {
        console.error("Error parsing JSON:", error);
        return prevContent;
  ...
  return (
    <div className="">
      <Card className="w-full mb-8">
        <CardHeader>
          <CardTitle>Hot News Section Preview</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="relative w-full h-[400px] bg-black">
            ...
            <div className="absolute inset-0 flex items-center flex-col justify-center">
              ...
              <span>{content ? JSON.parse(content) : ""}</span>
            </div>
          </div>
        </CardContent>
      </Card>
